...
ichnofossil_names <- ichnofossils$taxon_name[ichnofossils$reworking_mode %in% c("biodiffuser", "conveyor", "epifauna", "regenerator", "surficial")]

for (n in 1:length(ichnofossil_names)) {
  
  #select sentences with target words
  target <- ichnofossil_names[n]

  #select document id from sentences file
  doc_ids <- dbGetQuery(con, paste("SELECT DISTINCT docid FROM sentences WHERE ARRAY [\'", target, "\'] && words", sep=""))
  ...
  for (i in 1:nrow(doc_ids)) {
    ...
    if (length(strat_sentences) > 0)
    {
      ...
      target_sentences <- doc_sentences[grep(target, doc_sentences$words),]
...
write.csv(final_results, "ichnofossil_results.csv")
